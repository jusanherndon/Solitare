...
- **Local prototypes:** `cd prototype/klondike-table-flutter && flutter build apk` (Flutter SDK + Android SDK) and `flutter build ios --simulator` on macOS + Xcode (`build/ios/iphonesimulator/Runner.app`). Physical iPhone: free Apple ID Team + Developer Mode; paid program is store-only.
