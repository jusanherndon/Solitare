...
**Loss.** Not a win, and all of these hold: no **active Hint** (no legal face-up play Hint would show as new — repeats do not count, and a reverse-stop does not count while Hint hides it), and no Stock or Waste card that **draw or recycle can turn up as the Waste top** and that then plays onto the **current** Tableau or a Foundation. Face-down Stock still counts when a later tap can make it the Waste top. A buried draw-three card that the stride never turns up does **not** block a loss. A Tableau-run rehome Hint would otherwise skip **does** block a loss: Hint shows those last, preferring a run onto a King, when no useful new play and no drawable Waste top remain. A Foundation card other than an Ace onto Tableau is the same. Leftover Stock or Waste that cannot play does not hide those last-resort plays or a reverse-stop. King-empty hops and Foundation Ace-downs do **not** block a loss. Dimmed **Hint** is not itself a loss.
