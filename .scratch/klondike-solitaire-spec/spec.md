...
- **Local prototypes:** `cd prototype/klondike-table-flutter && flutter build apk` (Flutter SDK + Android SDK) and `flutter build ios --simulator` on macOS + Xcode (`build/ios/iphonesimulator/Runner.app`). Physical iPhone, unplug-and-play: `flutter build ipa --export-method development` then USB `flutter install --use-application-binary=build/ios/ipa/klondike_table.ipa`; free Apple ID Team + Developer Mode + trust developer; 7-day profile. Paid program is store-only.
