...
**Loss.** Not a win, and all of these hold: no **active Hint** (no legal face-up play Hint would treat as new — repeats do not count), no unused Tableau-run shift onto another pile that would leave an unseen face-up table (a King already on an empty pile hopping to another empty pile does not count), and no Stock or Waste card that **draw or recycle can turn up as the Waste top** and that then plays onto the **current** Tableau or a Foundation. Face-down Stock still counts when a later tap can make it the Waste top. A buried draw-three card that the stride never turns up does **not** block a loss. Dimmed **Hint** is not itself a loss.
