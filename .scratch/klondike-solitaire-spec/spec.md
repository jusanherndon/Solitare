...
**Loss.** Not a win, and all of these hold: no **active Hint** (no legal face-up play Hint would show as new — repeats do not count, and a reverse-stop does not count while Hint hides it), and no Stock or Waste card that **draw or recycle can turn up as the Waste top** and that then plays onto the **current** Tableau or a Foundation. Face-down Stock still counts when a later tap can make it the Waste top. A buried draw-three card that the stride never turns up does **not** block a loss. A last-resort Tableau-run rehome or Foundation pull other than an Ace-down **does** block a loss only when that play exposes a face-down card, makes Stock or Waste playable, or unlocks a new greedy Hint. King-empty hops, Foundation Ace-downs, and last-resort churn that cannot uncover a follow-up do **not** block a loss. Dimmed **Hint** is not itself a loss.
...
**Auto-move** only on double-tap / double-click: Foundation first if legal, otherwise a Tableau pile that **Hint** would show for that card — including a last-resort rehome or Foundation pull Hint is showing. Not Foundation onto Foundation. Not a Foundation Ace onto Tableau. Not a built Tableau run onto another pile unless Hint would show that play. Cards never fly to a Foundation on their own except after **Finish**.
...
The Hint cycle is rebuilt after a Tableau or Foundation play, Auto-move, Undo, a Stock draw, or a recycle, then restarted at the first **new** Hint (a play that would leave an unseen face-up table). A Hint tap does not rebuild. New plays first, repeats last. Within each group: Foundation destinations before Tableau; sources Waste, then Foundations left to right, then Tableau left to right; shortest legal run first. Each source-and-destination pair is one step. Wrap after the last step. Hint does not show the reverse of a Waste, Tableau, or Foundation play while that card still sits where the play put it. Draws and later plays of other cards do not bring the reverse back. If every remaining new play is such a reverse and draw or recycle cannot turn up a Waste top that plays, Hint still shows those new plays. If no useful new play remains and draw or recycle cannot turn up a Waste top that plays, Hint then shows last-resort Tableau-run rehomes (King destinations and longer runs first) and Foundation pulls other than Ace-downs **that expose a face-down card, make Stock or Waste playable, or unlock a new greedy Hint** — otherwise the button dims, and a **loss** does not treat a hidden reverse, a King-empty hop, a Foundation Ace-down, or last-resort churn as an **active Hint**.
...
**Win overlay.** Table visible around a centered dark rounded card with a gold border. **You won!** in gold. **Start**, then filled **New Game**, then **Winning deal** next to **New Game**. Dim the table (~40%). Gold sparkles rise through the open felt. While **Debug** is on, **Copy moves** sits under **Start**.
...
- **Local builds:** `cd src/klondike-table-flutter && flutter build apk` (Flutter SDK + Android SDK) and `flutter build ios --simulator` on macOS + Xcode (`build/ios/iphonesimulator/Runner.app`). Physical iPhone, unplug-and-play: `flutter build ipa --export-method development` then USB `flutter install --use-application-binary=build/ios/ipa/klondike_table.ipa`; free Apple ID Team + Developer Mode + trust developer; 7-day profile. Paid program is store-only.

Sources: [Which one-codebase toolkit should a beginner use to ship Klondike Solitaire?](https://github.com/jusanherndon/Solitare/issues/2), [Which Flutter analyzer, lint, and format tools should this repo use?](issues/13-flutter-lint-and-style-tools.md), [Can we build and install the Klondike table prototype on Android locally?](issues/06-android-local-prototype-build.md), [Can we build and install the Klondike table prototype on iOS locally?](issues/07-ios-local-prototype-build.md), [How do we install the Klondike table prototype on a physical iPhone without a connected `flutter run`?](issues/19-ios-disconnected-device-install.md).
