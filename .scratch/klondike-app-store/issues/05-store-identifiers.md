...
Shared with [What bundle ID and application ID does v1 use?](../klondike-play-store/issues/06-store-identifiers.md) — one reverse-DNS id for both stores; claim and resolve both together.
...
Play: the package name is **fixed** once the first artifact is uploaded (closed-test AAB counts). Lock this before that upload.
