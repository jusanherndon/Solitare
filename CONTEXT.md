...
**Loss**:
A Game that is not a win: no active Hint (a new play Hint would show — a reverse-stop does not count while Hint hides it; Hint still shows that reverse, or a last-resort Tableau-run rehome or Foundation pull other than an Ace-down when draw or recycle cannot help **and** that play exposes a face-down card, makes Stock or Waste playable, or unlocks a new greedy Hint), and no Stock or Waste card that draw or recycle can turn up as the Waste top and play onto the current Tableau or a Foundation. A buried draw-three card the stride never turns up does not block a loss. Repeats of a seen face-up table do not block a loss. King-empty hops, Foundation Ace-downs, and last-resort churn that cannot uncover a follow-up do not block a loss.
